...
1. 当terminal发送SIGINT或SIGTSTP时，是发给整个前台进程组，所以在用kill函数的时候第一个参数是`-pid`。

2. 在fork之前，主进程需要先用`sigprocmask`函数来block信号SIGCHLD，然后再unblock，这样做的目的是防止`addjob`和`deletejob`乱序执行。在某种调度顺序下，子进程先完成任务，发送SIGCHLD给父进程，父进程于是执行handler来处理，其中需要调用`deletejob`，这个时候`addjob`可能没有执行。注意在子进程也要unblock这个信号。

3. 需要修改子进程的进程组ID。fork出来的子进程和父进程的进程组ID是相同的，所以我们按下ctrl+c后，会同时发给我们的shell和shell的子进程，包括后台进程，这显然不是正确的。预期是只发给shell，然后由handler来处理这个信号。所以在`execve`之前需要调用`setpgid(0, 0)`，这个函数把当前进程的进程组id设置为当前进程的进程号。
